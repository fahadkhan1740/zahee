...
class CustomersController extends Controller
{
            ...
            if ($customer->role_id != 2) {
                ...
                if ($record->value == 'Maintenance' && $customer->role_id == 1) {
                    ...
                    return redirect('login')->with(
                        'loginError',
                        Lang::get("website.You Are Not Allowed With These Credentials!")
                    );
                }
            }
    ...
    public function Compare()
    {
        ...
        foreach ($compare as $com) {
            $data = array(
                'products_id' => $com->product_ids, 'page_number' => '0', 'type' => 'compare', 'limit' => '15',
                'min_price' => '', 'max_price' => ''
            );
    ...
    public function loadMoreWishlist(Request $request)
    {
        ...
        $data = array(
            'page_number' => $request->page_number, 'type' => 'wishlist', 'limit' => $limit, 'categories_id' => '',
    ...
    public function forgotPassword()
    {
        ...
        $result = array();
        $result['commonContent'] = $this->index->commonContent();
        return view("web.forgotpassword", ['title' => $title, 'final_theme' => $final_theme])->with(
            'result',
            $result
        );
    }
    ...
    public function processPassword(Request $request)
    {
        ...
        if (count($existUser) > 0) {
            ...
            return redirect('login')->with(
                'success',
                Lang::get("website.Password has been sent to your email address")
            );
        }
    ...
    public function signupProcess(Request $request)
    {
        ...
        if ($res['email'] == "true") {
            return redirect('/login')->withInput($request->input())->with(
                'error',
                Lang::get("website.Email already exist")
            );
        }
        if ($res['insert'] == "true") {
            if ($res['auth'] == "true") {
            ...
            return redirect('login')->with(
                'loginError',
                Lang::get("website.Email or password is incorrect")
            );
        }
