...
class AdminController extends Controller
{
        ...
        foreach ($orders as $orders_data) {
            ...
            if ($orders_status_history->orders_status_id != 3 and $orders_status_history->orders_status_id != 4) {
                foreach ($orders_products as $orders_product) {
                    ...
                    $single_purchased_price = DB::table('inventory')->where(
                        'products_id',
                        $orders_product->products_id
                    )->sum('purchase_price');
                    $single_stock = DB::table('inventory')->where(
                        'products_id',
                        $orders_product->products_id
    ...
    public function deleteadmin(Request $request)
    {
        ...
        DB::table('users')->where('id', '=', $_REQUEST['id'])->delete();
    ...
    public function categoriesroles(Request $request)
    {
        ...
        $categories_role = DB::table('users')->join(
            'categories_role',
            'categories_role.admin_id',
            '=',
            'users.role_id'
        ...
        foreach ($categories_role as $categories) {
            ...
            $categories_descrtiption = DB::table('categories_description')->whereIn(
                'categories_id',
                $cat_array
