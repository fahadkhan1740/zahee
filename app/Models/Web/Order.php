...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        ...
        $country = DB::table('countries')->where(
            'countries_id',
            '=',
        ...
        if ($payment_status == 'success') {
            ...
            $responseData = array(
                'success' => '1', 'data' => array(), 'message' => "Order has been placed successfully."
            );
    ...
    public function orders($request)
    {
        ...
        //orders
        $orders = DB::table('orders')->LeftJoin(
            'orders_products',
        ...
        if (count($orders) > 0) {
            ...
            foreach ($orders as $orders_data) {
                ...
                foreach ($order_products as $products) {
                    ...
                    $attributes = DB::table('orders_products_attributes')->where([
                        ['orders_id', $products->orders_id], ['orders_products_id', $products->orders_products_id]
                    ])->get();
    ...
    public function getPages($slug)
    {
        $pages = DB::table('pages')
            ...
            ->where([
                ['pages.status', '1'], ['type', 2], ['pages_description.language_id', session('language_id')],
                ['pages.slug', $slug]
            ])
            ->orwhere([
                ['pages.status', '1'], ['type', 2], ['pages_description.language_id', Session::get('language_id')],
                ['pages.slug', $slug]
            ])
            ->get();
        return $pages;
    ...
    public function payments_setting_for_brain_tree()
    {
        $payments_setting = DB::table('payment_methods_detail')
            ->leftjoin(
                'payment_description',
                'payment_description.payment_methods_id',
                '=',
                'payment_methods_detail.payment_methods_id'
            )
            ->leftjoin(
                'payment_methods',
    ...
    {
        $shippings_detail = DB::table('shipping_description')->where(
            'language_id',
    ...
    {
        $priceByWeight = DB::table('products_shipping_rates')->where('weight_from', '<=', $weight)->where(
            'weight_to',
            '>=',
            $weight
        )->get();
    ...
    {
        $braintree_description = DB::table('payment_description')->where([
            ['payment_name', 'Braintree'], ['language_id', Session::get('language_id')]
        ])
    ...
    {
        $stripe_description = DB::table('payment_description')->where([
            ['payment_name', 'Stripe'], ['language_id', Session::get('language_id')]
        ])
    ...
    {
        $cod_description = DB::table('payment_description')->where([
            ['payment_name', 'Cash On Delivery'], ['language_id', Session::get('language_id')]
        ])
    ...
    {
        $paypal_description = DB::table('payment_description')->where([
            ['payment_name', 'Paypal'], ['language_id', Session::get('language_id')]
        ])
    ...
    {
        $instamojo_description = DB::table('payment_description')->where([
            ['payment_name', 'Instamojo'], ['language_id', Session::get('language_id')]
        ])
    ...
    {
        $hyperpay_description = DB::table('payment_description')->where([
            ['payment_name', 'hyperpay'], ['language_id', Session::get('language_id')]
        ])
    ...
    {
        $ordersCheck = DB::table('orders')->where(
            ['customers_id' => auth()->guard('customer')->user()->id],
